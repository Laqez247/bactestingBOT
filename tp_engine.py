...
class TPEngine:
    ...
    # ------------------------------------------------------------------
    # CS Strategy TP generation — with new candidate types + round bonuses
    # ------------------------------------------------------------------
    ...
    def generate_cs(
        ...
        structure_engine,
        liquidity_engine,
        candidate_types: list = None,
        tp_min_score: float = 20,
        tp_min_rr: float = 1.0,
    ) -> List[TPCandidate]:
        """
        CS-specific TP generation. Accepts a list of candidate type names to evaluate.
        Adds $50/$100 round-number bonuses per spec.
        Returns sorted list of TPCandidate (highest score first) filtered by tp_min_score
        and tp_min_rr. An empty list means no valid TP → trade should be rejected.
        """
        atr_val = float(atr.iloc[i]) if not pd.isna(atr.iloc[i]) else 1.0
        sl_dist = abs(entry_price - sl_price)

        types = candidate_types or [
        ...
        for ct in types:
            if ct == "range_midpoint":
                cand = self._cs_range_midpoint(direction, entry_price, range_state)
                if cand:
                    candidates.append(cand)

            elif ct == "range_opposite_boundary":
                cand = self._cs_range_opposite(direction, entry_price, range_state)
                if cand:
                    candidates.append(cand)

            elif ct == "nearest_swing_in_direction":
                sw = self._swing_targets(direction, entry_price, i, structure_engine)
                candidates.extend(sw)

            elif ct == "round_number_in_direction":
                rn = self._round_number_targets(direction, entry_price, atr_val)
                candidates.extend(rn)

            elif ct == "nearest_other_round_number":
                rn = self._nearest_other_round_number(direction, entry_price, atr_val)
                candidates.extend(rn)

            elif ct == "session_high_low_in_direction":
                sess = self._session_target(direction, entry_price, liquidity_engine)
                if sess:
                    candidates.append(sess)

            elif ct == "pdh_pdl_in_direction":
                pdh = self._pdh_pdl(direction, entry_price, liquidity_engine)
                if pdh:
                    candidates.append(pdh)

            elif ct == "bot_range_boundary_in_direction":
                cand = self._bot_range_boundary(direction, entry_price, range_state)
                if cand:
                    candidates.append(cand)

        # Apply $50/$100 round-number TP bonuses (CS2 spec)
        major_tp_bonus = self._p("CS2_MAJOR_TP_BONUS", 15)
        minor_tp_bonus = self._p("CS2_MINOR_TP_BONUS", 10)
        major_inc      = self._p("CS2_MAJOR_INCREMENT", 100.0)
        minor_inc      = self._p("CS2_INCREMENT", 50.0)

        for c in candidates:
            level = c.price
            if level > 0:
                if level % major_inc == 0:
                    c.score += major_tp_bonus
                elif level % minor_inc == 0:
                    c.score += minor_tp_bonus
        ...
        # Filter using CS-specific thresholds
        max_rr = self._p("TP_MAX_RR", 4.0)
        candidates = [
            c for c in candidates
            if c.score >= tp_min_score and c.rr >= tp_min_rr
        ]
    ...
    def _cs_range_midpoint(self, direction: str, entry_price: float,
                           range_state) -> Optional[TPCandidate]:
        """Midpoint of the bot's detected range as TP target."""
        if not range_state.valid:
            return None
        rh = range_state.range_high
        rl = range_state.range_low
        mid = (rh + rl) / 2.0
        if direction == "LONG" and mid <= entry_price:
            return None
        if direction == "SHORT" and mid >= entry_price:
            return None
        return TPCandidate(price=mid, score=30, target_type="RANGE_MIDPOINT")

    def _cs_range_opposite(self, direction: str, entry_price: float,
                           range_state) -> Optional[TPCandidate]:
        """Opposite range boundary as TP (full range width)."""
        if not range_state.valid:
            return None
        if direction == "LONG":
            target = range_state.range_high
            if target <= entry_price:
                return None
        else:
            target = range_state.range_low
            if target >= entry_price:
                return None
        return TPCandidate(price=target, score=25, target_type="RANGE_OPPOSITE_BOUNDARY")

    def _bot_range_boundary(self, direction: str, entry_price: float,
                            range_state) -> Optional[TPCandidate]:
        """Bot's detected range boundary in the trade direction."""
        if not range_state.valid:
            return None
        if direction == "LONG":
            target = range_state.range_high
            ...
            if target >= entry_price:
                return None
        return TPCandidate(price=target, score=22, target_type="BOT_RANGE_BOUNDARY")

    def _nearest_other_round_number(self, direction: str, entry_price: float,
                                    atr_val: float) -> List[TPCandidate]:
        """
        Next $50 level in the trade direction (CS2 TP candidate).
        Distinct from round_number_targets — returns the nearest one only.
        """
        increment = self._p("CS2_INCREMENT", 50.0)
        proximity = self._p("ROUND_NUMBER_PROXIMITY", 5)
        cands = []
        search_range = atr_val * 10
        start_price  = entry_price - search_range
        ...
        while level <= end_price:
            if direction == "LONG" and level > entry_price + proximity:
                cands.append(TPCandidate(price=level, score=18,
                                         target_type="NEXT_ROUND_NUMBER"))
            elif direction == "SHORT" and level < entry_price - proximity:
                cands.append(TPCandidate(price=level, score=18,
                                         target_type="NEXT_ROUND_NUMBER"))
            level += increment
        ...
        if direction == "LONG":
            return [min(cands, key=lambda c: c.price)]
        return [max(cands, key=lambda c: c.price)]
