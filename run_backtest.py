...
from structure_engine import (
    StructureEngine, compute_atr, run_htf_structure, align_htf_to_exec,
    compute_htf_rsi, align_htf_rsi_to_exec,
)
from strategies.complementary_runner import run_complementary_checks
...
def run_single_backtest(
    ...
    if p("SESSION_FILTER_ON", True):
        ...
        # Extend valid_hours for CS strategies that trade NY session.
        # If any CS strategy is enabled and CS_SESSION_FILTER includes "new_york",
        # NY bars must be processed so the CS runner can fire during NY.
        # NY hours = 12:00–17:00 UTC.
        _any_cs = any([
            p("CS1_ENABLED", True), p("CS2_ENABLED", True),
            p("CS3_ENABLED", True), p("CS4_ENABLED", True),
        ])
        _cs_filter = p("CS_SESSION_FILTER", ["london", "new_york"])
        if _any_cs and "new_york" in _cs_filter and not p("STRATEGY_ONLY_EDGE2", False):
            for _h in range(12, 17):  # NY session: 12:00–16:59 UTC
                valid_hours.add(_h)
    ...
    # Compute HTF RSI(14) for CS4 — aligned forward-fill onto exec TF
    htf_rsi_series = compute_htf_rsi(df_htf, period=14)
    htf_rsi_aligned = align_htf_rsi_to_exec(htf_rsi_series, df_exec.index)
    ...
    # Default regime (overwritten each session bar)
    ...
    for i in range(n):
        ...
        # --- Compute regime once per session bar (shared by CS + Edge 2) ---
        try:
            _htf_i = df_htf.index.searchsorted(ts, side="right") - 1
            _htf_i = max(0, min(_htf_i, len(df_htf) - 1))
            regime_info = _detect_regime(
                df_htf=df_htf,
                current_htf_idx=_htf_i,
                lookback_bars=120,
            )
        except Exception:
            regime_info = {"regime": "RANGING", "htf_swing_count": 0,
        ...
        # --- CS STRATEGY CHECK (runs before Edge 2 gate chain) ---
        # Only when no trade is open and CS strategies are enabled
        _cs_enabled = any([
            p("CS1_ENABLED", True), p("CS2_ENABLED", True),
            p("CS3_ENABLED", True), p("CS4_ENABLED", True),
        ])
        if _cs_enabled and not p("STRATEGY_ONLY_EDGE2", False):
            try:
                cs_signal = run_complementary_checks(
                    range_state=range_eng.state,
                    liquidity_engine=liq_eng,
                    structure_engine=structure_eng,
                    htf_aligned_series=htf_aligned,
                    htf_rsi_series=htf_rsi_aligned,
                ...
                if cs_signal is not None and cs_signal.tp_candidates:
                    cs_rec = _open_cs_trade(
                        signal=cs_signal,
                        i=i,
                        df=df_exec,
                        atr_i=atr_i,
                    ...
                    if cs_rec is not None:
                        # CS trade opened — skip Edge 2 gate chain this bar
                        continue
            except Exception:
                pass  # CS errors must never crash the main Edge 2 loop
        ...
        if zone_eng.has_valid_zone:
            ...
            if zone_reaction_confirmed and breakout_eng.current_breakout is not None:
                ...
                # regime_info already computed at top of session-bar block (shared with CS)
...
def _open_cs_trade(
    signal,            # CSSignal from complementary_runner
    i: int,
    df: pd.DataFrame,
    atr_i: float,
    ts,
    iteration: int,
    sim,               # TradeSimulator instance
    cfg: dict,
) -> "Optional[TradeRecord]":
    """
    Open a trade from a CS signal.
    Creates a TradeRecord, registers it as the active trade in the simulator,
    and returns it. Returns None if trade could not be opened.

    CS trades bypass the full Edge 2 gate chain — their signals already
    encode direction, entry, SL, and TP candidates from their own gate sequence.
    """
    if not signal.tp_candidates:
        return None
    ...
    sl_max_atr = cfg.get("SL_MAX_DISTANCE_ATR", getattr(config, "SL_MAX_DISTANCE_ATR", 2.5))
    if atr_i > 0 and sl_dist / atr_i > sl_max_atr:
        return None

    # Select TP1 (highest score candidate)
    tp1 = signal.tp_candidates[0]
    if tp1.rr < 1.0:
        return None  # minimum RR not met

    # Select TP2 if dual TP enabled and a second candidate exists
    tp2 = None
    dual_tp = cfg.get("DUAL_TP_ENABLED", getattr(config, "DUAL_TP_ENABLED", True))
    if dual_tp and len(signal.tp_candidates) >= 2:
        min_tp2_rr = cfg.get("TP2_MIN_RR", getattr(config, "TP2_MIN_RR", 1.8))
        for cand in signal.tp_candidates[1:]:
            if cand.rr >= min_tp2_rr:
                tp2 = cand
                break

    # Fixed TP1 at 1R if configured (CS strategies also respect this setting)
    tp1_fixed_rr = cfg.get("TP1_FIXED_RR", getattr(config, "TP1_FIXED_RR", 1.0))
    if tp1_fixed_rr > 0:
        if signal.direction == "LONG":
            tp1_price = signal.entry_price + sl_dist * tp1_fixed_rr
        else:
            tp1_price = signal.entry_price - sl_dist * tp1_fixed_rr
        tp2_price = tp1.price  # dynamic target becomes TP2
        tp2_rr    = tp1.rr
        tp1_rr    = tp1_fixed_rr
    else:
        tp1_price = tp1.price
        tp1_rr    = tp1.rr
        tp2_price = tp2.price if tp2 else 0.0
        tp2_rr    = tp2.rr   if tp2 else 0.0

    # Get session label
    session = "unknown"
    hour = ts.hour
    sessions_cfg = cfg.get("TRADE_SESSIONS", getattr(config, "TRADE_SESSIONS", {}))
    for name, window in sessions_cfg.items():
        ...
        if oh <= hour < ch:
            session = name
            break
    # Also catch NY (CS strategies allow NY)
    if session == "unknown" and 12 <= hour < 17:
        session = "new_york"

    sim._setup_counter = getattr(sim, "_setup_counter", 0) + 1
    setup_id = f"CS_{signal.strategy}_{sim._setup_counter:05d}"

    rec = TradeRecord(
        setup_id=setup_id,
        iteration=iteration,
        symbol=cfg.get("SYMBOL", getattr(config, "SYMBOL", "XAU/USD")),
        timeframe=cfg.get("EXECUTION_TF", getattr(config, "EXECUTION_TF", "5min")),
        direction=signal.direction,

        entry_price=round(signal.entry_price, 2),
        sl_price=round(signal.sl_price, 2),
        original_sl_price=round(signal.sl_price, 2),
        tp1_price=round(tp1_price, 2),
        tp2_price=round(tp2_price, 2),

        tp_score=round(tp1.score, 1),
        rr_tp1=round(tp1_rr, 2),
        rr_tp2=round(tp2_rr, 2),
        ...
        atr_at_setup=round(atr_i, 4),

        # CS strategy identification
        strategy_type=signal.strategy,
        modification_type=signal.modification_type,
        cs_setup_context=signal.setup_context,
    )

    sim._active_trade = rec
    return rec
